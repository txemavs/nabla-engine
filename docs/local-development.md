...
position in the tile frame). The viewer uses these for world labels and GPS locality readouts without adding
editor entities or physics bodies. Labels are displayed 1,000m above terrain and
release their textures when tiles are evicted. The active non-overlapping tile
cover owns the metadata. Older manifests without this optional field still load normally.
