...
test('six presets persist their visual settings and cap speculative demand', async ({ page }) => {
  ...
  expect(values[5]).toMatchObject({ preset: 'extreme', distance: 50000, cache: 250, concurrent: 4 })
  await page.reload()
  const saved = await page.evaluate(async () =>
    (await import(String('/performance.ts'))).readPerformance(),
  )
  expect(saved.preset).toBe('extreme')
  expect(saved.distance).toBe(50000)
})
