...
test('places objects and portals at a persistent cursor with exact axis movement', async ({
  ...
  await expect(page.getByLabel('Posición local · m X', { exact: true })).toHaveValue('12')
  await expect(page.getByLabel('Posición local · m Y', { exact: true })).toHaveValue('2')
  await page.locator('[popovertarget="cursor-menu"]').click()
  ...
  await page.keyboard.press('Escape')
  await expect(page.getByLabel('Posición local · m X', { exact: true })).toHaveValue('14.75')
  ...
  const portal = saved.entities.find((e: { name: string }) => e.name === 'Stargate · origen')
  expect(portal.transform.position).toEqual([12, 3.455, -8])
