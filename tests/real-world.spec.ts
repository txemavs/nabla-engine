...
test('starts in Ventas, saves authored content without generated entities and drives on native GLBs', async ({
  ...
  // The prompt can appear before the suspension finishes its first settling step.
  await expect(async () => {
    if ((await page.locator('#player-mode').textContent()) !== 'MI A3')
      await page.keyboard.press('KeyE')
    await expect(page.locator('#player-mode')).toHaveText('MI A3', { timeout: 500 })
