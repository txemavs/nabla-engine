...
export class WheelDebugOverlay {
  ...
  update(
    ...
    const wheelTransforms = sim.wheelTransforms(vehicleId, true)
    this.lastData = []
    ...
    for (let i = 0; i < Math.min(wheelContacts.length, 4); i++) {
      ...
      if (contact.contactPoint) {
        ...
        this.raycaster.set(
          new THREE.Vector3(wheelCenter.x, wheelCenter.y + 2, wheelCenter.z),
        ...
        if (visualHit) {
          ...
          linePositions.setXYZ(0, marker.physics.position.x, marker.physics.position.y, marker.physics.position.z)
          linePositions.setXYZ(1, marker.visual.position.x, marker.visual.position.y, marker.visual.position.z)
